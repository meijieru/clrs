...
void matrix_strassen_iter(int (*source1)[],int (*source2)[],int (*destination)[],int size);
void matrix_strassen(int (*source)[],int lower_bound,int high_bound);
void matrix_add(int (*source1)[],int (*source2)[],int (*destination)[],int low_bound1,int high_bound1,int lower_bound2,int high_bound2);
void matrix_sub(int (*source)[],int (*destination)[],int size,int low_bound,int high_bound);
...
void matrix_strassen_iter(int (*source1)[],int (*source2)[],int (*destination)[],int size)
{
    int mid = size/2;
    int left = size - mid;
    int *ptr_source1 = (int *)source1,*ptr_source2 = (int *)source2,*ptr_destination = (int *)destination;
...
void matrix_add(int (*source1)[],int (*source2)[],int (*destination)[],int low_bound1,int high_bound1,int low_bound2,int high_bound2)
{
    int i,j,k;
    int *ptr_source1,*ptr_source2,*ptr_destination;
    int size;

    if((high_bound1-low_bound1)!=(high_bound2-low_bound2))
        exit(1);
