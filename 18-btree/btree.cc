...
class BTree {
 public:
  ...
  int IsLeaf(Node *p) { return p->isleaf; }
  ...
  pair<Node *, int> Min(void) { return Min(root_); }
  ...
  pair<Node *, int> Max(void) { return Max(root_); }
  ...
  void DeleteNonone(Node *p, int k) {
    ...
    if (IsLeaf(p)) {
      ...
      } else if (p->kids[i]->key.size() == t_ - 1) {
        // case 3
        Node *ptr = p->kids[i];
