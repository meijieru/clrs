...
vector<int> GreedyActivitySelectorRecursive(vector<int> s, vector<int> f, int k,
                                            int n) {
  int m = k + 1;
  while ((m <= n) && (s[m] < f[k])) {
    ++m;
...
void Huffman(vector<pair<char, int>> &c) {
  ...
  priority_queue<Node *, vector<Node *>, function<bool(Node * x, Node * y)>>
      min_heap(fun);
  for (int i = 0; i < n; ++i) {
  ...
  for (int i = 0; i < n - 1; ++i) {
    ...
    min_heap.pop();
    inserted->frequence =
        inserted->left->frequence + inserted->right->frequence;
    min_heap.push(inserted);
  }
...
int main() {
  ...
  vector<pair<char, int>> code{{'f', 5},  {'e', 9},  {'c', 12},
                               {'b', 13}, {'d', 16}, {'a', 45}};
  Huffman(code);
  // PrintTree(root,0);
  // Release(root);
