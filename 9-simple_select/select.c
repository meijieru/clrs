...
int main() {
  ...
  for (i = 0; i < SIZE; i++) printf("%d ", source[i]);  // notice the type
...
int Random_select(int *source, int left_index, int right_index, int power) {
  ...
  if (right_index == left_index) return source[left_index];
  ...
  if (tmp == power)
    ...
    return Random_select(source, left_index, mid_index - 1,
                         power);  // will not be call with the length of 0
  else
...
void Insertion(int *source, int left_index, int right_index) {
  ...
  for (i = 1; i <= size; i++) {
    ...
    for (k = left_index; k < right_index + 1; k++) printf("%d ", source[k]);
