...
void Max_Heapify(heap *source, int index) {
  ...
  while (1) {
    ...
    if ((left_index < size) &&
        (source->array[left_index] > source->array[index]))
      largest = left_index;
    else
      largest = index;

    if ((right_index < size) &&
        (source->array[right_index] > source->array[largest]))
      largest = right_index;
 ...
 * @return
 */
int Max_Heap_Maximum(heap *source) { return source->array[0]; }
...
int Max_Heap_Extract_Max(heap *source) {
  ...
  if (source->heap_size < 1) printf("heap underflow\n");
...
void Max_Heap_Insert(heap *source, int key) {
  if (source->heap_size >= source->length) printf("heap overflow");
...
void Max_Heap_Increase_Key(heap *source, int index, int key) {
  if (key < source->array[index]) printf("new key is smaller");
