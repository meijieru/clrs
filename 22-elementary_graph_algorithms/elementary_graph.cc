 ...
 public:
  ...
  void BFSPrint(int start, int end) {
    if (start == end) {
      ...
      cout << "there is no pace from " << start << " to " << end << endl;
    }
  }
  ...
  void DFSPrint(void) {
    for (auto &a : vertexs_) {
      cout << "start time:" << a->s << " end time:" << a->f
           << " parent:" << a->parent << endl;
    }
  }

  int OutDegree(int p) { return edges_[p].size(); }
  int InDegree(int p) {
    int count = 0;
    for (auto &a : edges_) {
      for (auto &b : a) {
...
int main() {
  ...
  lg.InsertEdge(5, 8, 1);
  lg.Print();
  cout << "OutDegree:" << lg.OutDegree(4) << " InDegree:" << lg.InDegree(4)
       << endl;
  lg.BFS(2);
  lg.BFSPrint(2, 9);
  cout << "-------------------------------" << endl;
