...
class DirectAddressing {
 public:
  ...
  ~DirectAddressing() { delete[] index_table_; }
...
class ChainningHash {
 public:
  ...
  void Insert(int target) {
    ...
    if (l.end() == find(l.begin(), l.end(),
                        target))  // if target already in the list,do nothing
      l.push_front(target);
  }
  ...
  int *Search(int target) {
    ...
    if (l.end() == it) return nullptr;
    return &*it;
  }

  void Delete(int target) {
    auto &l = T_[Hash(target)];
    for (auto it = l.begin(), last = l.before_begin(); it != l.end();
         it++, last++) {
      if (*it == target) {
        l.erase_after(last);
        return;
...
class OpenAddressHash {
 public:
  OpenAddressHash(size_t size) : size_(size) { T_.resize(size, kNil); }
  ...
  int *Search(int target) {
    ...
    for (int i = 0; i < m_ && T_[pos] != kNil; i++) {
      pos = Hash(target, i);
      if (T_[pos] == target) return &T_[pos];
    }
    return nullptr;
  }
  ...
  void Delete(int target) {
    for (int i = 0; i < m_; i++) {
      int pos = Hash(target, i);
      if (T_[pos] == target) T_[pos] = kDelete;
    }
  }

 private:
  int Hash1(int target) { return target % m_; }

  int Hash2(int target) { return target % m_; }
...
int main() {
  ...
  for (int i = 0; i < 100; i += 4) direct_addressing.Insert(i);
  for (int i = 0; i < 100; i += 4) cout << *direct_addressing.Search(i) << endl;
  direct_addressing.Delete(24);
  direct_addressing.Delete(96);
  int *loc;
  for (int i = 0; i < 100; i += 4)
    if ((loc = direct_addressing.Search(i)) != nullptr) cout << *loc << endl;
  cout << "tag1" << endl;

  ChainningHash chainning_hash(21, 5);
  for (int i = 0; i < 100; i += 4) chainning_hash.Insert(i);
  for (int i = 0; i < 100; i += 4) cout << *chainning_hash.Search(i) << endl;
  chainning_hash.Delete(96);
  chainning_hash.Delete(24);
  for (int i = 0; i < 100; i += 4)
    if ((loc = chainning_hash.Search(i)) != nullptr) cout << *loc << endl;
  cout << "tag2" << endl;

  OpenAddressHash open_address_hash(30);
  for (int i = 0; i < 100; i += 4) open_address_hash.Insert(i);
  for (int i = 0; i < 100; i += 4) cout << *open_address_hash.Search(i) << endl;
  open_address_hash.Delete(24);
  open_address_hash.Delete(96);
  for (int i = 0; i < 100; i += 4)
    if ((loc = open_address_hash.Search(i)) != nullptr) cout << *loc << endl;
  cout << "tag3" << endl;
