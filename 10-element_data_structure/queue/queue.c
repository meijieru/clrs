...
int main() {
  ...
  for (i = 0; i < QUEUE_SIZE; i++) Queue_enqueue(&queue, i);
  for (i = 0; i < QUEUE_SIZE; i++) printf("%d ", Queue_dequeue(&queue));
  printf("\n");
  Queue_delete(&queue);
...
void Queue_enqueue(Queue_ptr queue, int target) {
  if (queue->size >= queue->capacity) exit(2);
